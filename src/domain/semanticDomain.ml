...
module SemanticsDomain =
  struct
    ...
    *)
    ...
    let array_M env m = 
      let n_make = construct_vnode env (create_singleton_trace "Array.make") (create_singleton_trace "") in
      ...
      let t_make = (* make *)
        ...
        in
        let t = 
          let t' = Table (construct_table (create_singleton_trace var_e) (Relation rm, Ary ary)) in
          Table (construct_table (create_singleton_trace var_l) (Relation rl, t')) in
        t
      in
      let n_len = construct_vnode env  (create_singleton_trace "Array.length") (create_singleton_trace "") in
      ...
      let t_len = (* len *)
        ...
        Table (construct_table (create_singleton_trace var_l) (Ary ary, Relation rlen))
      in
      let n_get = construct_vnode env (create_singleton_trace "Array.get") (create_singleton_trace "") in
      ...
      let t_get = (* get *)
        ...
        let var_zi = "zi" in
        let t = 
          let t' = Table (construct_table (create_singleton_trace var_zi) (Relation rm, Relation rr)) in
          Table (construct_table (create_singleton_trace var_l) (Ary ary, t')) in
        t
      in
      let n_set = construct_vnode env (create_singleton_trace "Array.set") (create_singleton_trace "") in
      ...
      let t_set = (* set *)
        ...
        let rrr = [] in
        let t = 
          let t' = Table (construct_table (create_singleton_trace var_e) (Relation rri, Tuple rrr)) in
          let t'' = Table (construct_table (create_singleton_trace var_zi) (Relation rm, t')) in
          Table (construct_table (create_singleton_trace var_l) (Ary ary, t'')) in
        t
      in
      let m' = 
      ...
      env', m'
    let list_M env m = 
      let n_len = construct_vnode env (create_singleton_trace "List.length") (create_singleton_trace "") in
      let s_len = construct_snode "" n_len in
      let t_len = (* len *)
        ...
        (* let rl = op_R var_i "0" Ge true llen |> op_R var_i var_len Lt true in *)
        let rlen = equal_R (top_R Plus) "l" in
        Table (construct_table (create_singleton_trace var_l) (Lst list, Relation rlen))
      in
      let n_hd = construct_vnode env (create_singleton_trace "List.hd") (create_singleton_trace "") in
      ...
      let t_hd = (* hd *)
        ...
        let tr = Top in
        let t =
          Table (construct_table (create_singleton_trace var_h) (Lst list, tr)) in
        t
      in
      let n_tl = construct_vnode env (create_singleton_trace "List.tl") (create_singleton_trace "") in
      ...
      let t_tl = (* tl *)
        ...
        in
        let t =
          Table (construct_table (create_singleton_trace var_t) (Lst list1, Lst list2)) in
        t
      in
      let n_cons = construct_vnode env (create_singleton_trace "List.cons") (create_singleton_trace "") in
      ...
      let t_cons = (* cons *)
        ...
        in
        let t =
          let t' = Table (construct_table (create_singleton_trace var_l) (Lst list1, Lst list2)) in
          Table (construct_table (create_singleton_trace var_c) (ve, t')) in
        t
      in
      let m' = 
      ...
      let m', env' =
        if VarDefMap.is_empty !pre_vars then
          ...
          VarDefMap.fold (fun var (domain: pre_exp) (m, env) -> 
            let n_var = construct_vnode env (create_singleton_trace var) (create_singleton_trace "") in
