...
module type SemanticsType =
  sig
    ...
    val prop_table: (trace_t -> value_t * value_t -> value_t * value_t -> (value_t * value_t) * (value_t * value_t)) -> (value_t -> var -> var -> value_t) -> table_t -> table_t -> table_t * table_t
    val step_func: (trace_t -> value_t * value_t -> 'a -> 'a) -> value_t -> 'a -> 'a
    val get_full_table_T: table_t -> trace_t * (value_t * value_t)
    val get_table_by_cs_T: trace_t -> table_t -> (value_t * value_t)
    val update_table: trace_t -> value_t * value_t -> table_t -> table_t
    val table_isempty: table_t -> bool
    val table_mapi: (trace_t -> value_t * value_t -> value_t * value_t) -> table_t -> table_t
  end
...
module NonSensitive: SemanticsType =
  struct
    ...
    let init_T trace = get_trace_data trace, Bot, Bot
    ...
    let prop_table f g (t1:table_t) (t2:table_t) = 
      ...
      let (z1, v1i, v1o) = t1' and (z2, v2i, v2o) = t2' in
      let z_tr1 = create_singleton_trace z1 in
    ...
    let step_func f v m = 
      ...
      let z_tr = create_singleton_trace z in
      f z_tr (tl, tr) m
    let get_full_table_T t = let (z, vi, vo) = t in
      create_singleton_trace z, (vi, vo)
