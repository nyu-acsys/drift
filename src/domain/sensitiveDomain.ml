...
module type SensitiveSemanticsType =
  sig
    ...
    val construct_table: (var * loc) -> value_te * value_te -> table_t
    val io_T: (var * loc) -> value_te -> value_te * value_te
    val get_vnode: node_t -> env_t * loc * (var * loc)
    val dx_T: value_te -> (var * loc)
    ...
    val prop_table: ((var * loc) -> value_te * value_te -> value_te * value_te -> (value_te * value_te) * (value_te * value_te)) -> (value_te -> string -> string -> value_te) -> table_t -> table_t -> table_t * table_t
    val step_func: ((var * loc) -> value_te * value_te -> 'a -> 'a) -> value_te -> 'a -> 'a
    val get_full_table_t: table_t -> (var * loc) * (value_te * value_te)
    val get_table_by_cs_T: (var * loc) -> table_t -> (value_te * value_te)
    val update_table: (var * loc) -> value_te * value_te -> table_t -> table_t
    val table_isempty: table_t -> bool
    val table_mapi: ((var * loc) -> value_te * value_te -> value_te * value_te) -> table_t -> table_t
  end
...
module OneSensitive: SensitiveSemanticsType =
  struct
    ...
    let io_T cs v = match v with
        ...
        | _ -> raise (Invalid_argument "Should be a table when using io_T")
    let dx_T v = match v with
        | TypeAndEff (Table t, _) -> let cs, _ = try TableMap.min_binding t with
          Not_found -> ("",""), (TEBot,TEBot) in cs
        | _ -> raise (Invalid_argument "Should be a table when using dx_T")
    let get_table_t = function
      | TypeAndEff (Table t, _) -> t
      | _ -> raise (Invalid_argument "Should be a table when using get_table_t")
...
(* module type SensitiveSemanticsType =
  sig
    ...
    type value_te = | Bot
      | Top
      | Relation of relation_te
      | Table of table_t
      | Ary of array_t
      | Unit of unit
    val init_T: var -> table_t
    val alpha_rename_T: (value_te -> string -> string -> value_te) -> table_t -> string -> string -> table_t
    val join_T: (value_te -> value_te -> value_te) -> table_t -> table_t -> table_t
    val meet_T: (value_te -> value_te -> value_te) -> table_t -> table_t -> table_t
    val leq_T: (value_te -> value_te -> bool) -> table_t -> table_t -> bool
    val eq_T: (value_te -> value_te -> bool) -> table_t -> table_t -> bool
    val forget_T: (var -> value_te -> value_te) -> var -> table_t -> table_t
    val arrow_T: (var -> value_te -> value_te) -> (var -> value_te -> value_te -> value_te) -> var -> table_t -> value_te -> table_t
    val wid_T: (value_te -> value_te -> value_te) -> table_t -> table_t -> table_t
    val equal_T: (value_te -> var -> value_te) -> table_t -> var -> table_t
    val replace_T: (value_te -> var -> var -> value_te) -> table_t -> var -> var -> table_t
    val stren_T: (value_te -> value_te -> value_te) -> table_t -> value_te -> table_t
    val proj_T: (value_te -> string array -> value_te) -> table_t -> string array -> table_t
