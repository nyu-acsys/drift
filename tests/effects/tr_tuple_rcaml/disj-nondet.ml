...
let main (v:int(*-:{v:Int | true}*)) (p:int(*-:{v:Int | true}*)) (n:int(*-:{v:Int | true}*)) =
  if p > 0 && n < 0 then
    let bb = if (nondet_bool ()) then 1 else 0 in
    f v bb p n
  else 0 
...
let rec f x b pos neg cfg2 =
  (match (if (b = 0) then let cfg4 = ((ev_step0 pos) cfg2) in 
...
let main (v:int(*-:{cur_v:Int | true = true}*)) (p:int(*-:{cur_v:Int | true = true}*)) (n:int(*-:{cur_v:Int | true = true}*)) =
  (if ((p > 0) && (n < 0)) then let bb = (if (nondet_bool ()) then 1
                                         else 0) in 
                                (((((f v) bb) p) n) (0,0))
  else (0,(0,0)))
