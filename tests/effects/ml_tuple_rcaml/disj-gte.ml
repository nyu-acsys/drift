(*
Tuple Encoding of Product Program.

Source Program: 

let rec f x pos neg =
  if x > 0 then
    ev pos
  else if x < 0 then  
    ev neg
  else 
    ();

  if x > 0 then
    f (x-1) pos neg
  else if x < 0 then 
    f (x+1) pos neg
  else
    0
  
let main (v:int(*-:{v:Int | true}*)) (p:int(*-:{v:Int | true}*)) (n:int(*-:{v:Int | true}*)) =
  if p > 0 && n < 0 then
    f v p n
  else 0 


Property: 

(* Disjunctive *)

QSet   = [0;1;2;3]; 

delta  = fun evx (q, acc) -> 
       	     if ((q = 0) || (q = 1)) && (evx > 0) then (1,acc)
	     else if (q = 1) && (evx < 0) then (3, acc)
       	     else if ((q = 0) || (q = 2)) && (evx < 0) then (2,acc)
	     else if (q = 2) && (evx > 0) then (3, acc)
	     else (3, acc);

IniCfg = (0, 0);

(* if event is pos, x must  be even ; *)
assert = fun (q, acc) -> (q < 3);



*)

let ev_step0 evx cfg0 = (match cfg0 with 
                         (q,acc) -> if (((q = 0) || (q = 1)) && (evx > 0)) then (1,acc)
                                    else if ((q = 1) && (evx < 0)) then (3,acc)
                                         else if (((q = 0) || (q = 2)) && (evx < 0)) then (2,acc)
                                              else if ((q = 2) && (evx > 0)) then (3,acc)
                                                   else (3,acc))


let ev_step_asst0 cfg1 = (match cfg1 with 
                          (q,acc) -> assert (q < 3))


let rec f x cfg2 = ((fun pos cfg3 ->
                       ((fun neg cfg4 ->
                           (match if (x > 0) then ((fun cfg6 ->
                                                      ((ev_step_asst0 cfg6) ; ((),cfg6))) ((ev_step0 pos) cfg4))
                                  else if (x < 0) then ((fun cfg5 ->
                                                           ((ev_step_asst0 cfg5) ; ((),cfg5))) ((ev_step0 neg) cfg4))
                                       else ((),cfg4) with 
                            (x0,cfg7) -> (match if
                                                  (x > 0)
                                                  then
                                                  (match (match ((f (x - 1)) cfg7) with 
                                                          (x4,cfg11) -> ((x4 pos) cfg11)) with  (x5,cfg12) -> ((x5 neg) cfg12))
                                                  else if (x < 0) then (match (match ((f (x + 1)) cfg7) with 
                                                                               (x2,cfg9) -> ((x2 pos) cfg9)) with  (x3,cfg10) -> ((x3 neg) cfg10))
                                                                       else (0,cfg7) with 
                                                  (x1,cfg8) -> ((x0 ; x1),cfg8)))),cfg3)),cfg2) 


let main (v:int(*-:{cur_v:Int | true = true}*)) (p:int(*-:{cur_v:Int | true = true}*)) (n:int(*-:{cur_v:Int | true = true}*)) = if
                                                                                                                                  ((p > 0) && (n < 0))
                                                                                                                                  then
                                                                                                                                  (match (match ((f v) (0,0)) with 
                                                                                                                                          (x6,cfg13) -> ((x6 p) cfg13)) with 
                                                                                                                                         (x7,cfg14) -> ((x7 n) cfg14))
                                                                                                                                  else (0,(0,0))

[@@@assert "typeof(main) <: int -> int -> int -> int * (int * int)"]
