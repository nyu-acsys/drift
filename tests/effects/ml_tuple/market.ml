...
let sell n1 cfg2 = (match ((fun cfg3 ->
...
let buy n2 cfg9 = (match ((fun cfg10 ->
...
let activity f cfg16 = ((fun n3 cfg17 ->
...
let rec repeat n4 cfg23 = if (n4 <= 0) then (0,cfg23)
                          else
                            (match (match ((activity buy) cfg23) with 
                                   ...
                                   (x13,cfg25) -> (match (match (match ((activity sell) cfg25) with 
                                                                ...
                                                                (x16,cfg28) -> (match (match ((repeat (n4 - 1)) cfg28) with 
                                                                                       (x18,cfg30) -> ((1 + x18),cfg30)) with 
                                                                                      (x17,cfg29) -> ((x16 ; x17),cfg29))) with 
                                                                  (x14,cfg26) -> ((x13 ; x14),cfg26))) 


let main (budget:int(*-:{cur_v:Int | cur_v = 0}*)) (stock:int(*-:{cur_v:Int | cur_v = 0}*)) (n:int(*-:{cur_v:Int | cur_v = 0}*)) = if
