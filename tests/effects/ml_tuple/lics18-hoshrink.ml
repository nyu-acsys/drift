...
let rec shrink t cfg2 = ((fun f cfg3 ->
                            ((fun d cfg4 ->
...
let main (gl_t:int(*-:{cur_v:Int | true = true}*)) (gl_d:int(*-:{cur_v:Int | true = true}*)) = (match (match (match (match ((shrink gl_t) ((ev_step0 (gl_t / gl_d)) (0,0))) with 
                                                                                                                     (x8,cfg15) -> ((x8 (fun x cfg16 ->
                                                                                                                                           (gl_t,cfg16))) cfg15)) with 
                                                                                                                    (x9,cfg17) -> ((x9 gl_d) cfg17)) with 
