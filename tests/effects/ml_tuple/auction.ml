...
let rec refund j cfg3 = if (j <= 0) then ((),cfg3)
                        else (match ((fun cfg4 ->
...
let close n cfg7 = if (n = 0) then ((),cfg7)
                   else (match ((fun cfg8 ->
...
let main (u:unit(*-:{cur_v:Unit | unit = unit}*)) = (match ((bid 0) (0,(0,0))) with 
