# Generate CPS translation or all pairs of programs and properties that match on the name
# input: 
...
def translate (drift_path, benches, dst):
    def tr_b (b, prg, prop):
        if (prop == None):
            ...
            tr_res = subprocess.run([drift_path, "-cps-convert", "true", "-file", prg, "-prop", prop], 
                                    check=True, capture_output=True)
            fw = open(os.path.join(dst, "cps_" + b + bench_prg_ext), "wb")
